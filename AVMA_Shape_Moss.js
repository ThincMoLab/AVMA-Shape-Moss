...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'media/shape_right_background.png', 'path': './media/shape_right_background.png'},
    {'name': 'media/shape_reverse_background.png', 'path': './media/shape_reverse_background.png'},
...
function experimentInit() {
  ...
  if (training_vol.length === 0 || isNaN(participant) || isNaN(session) || handedness.length === 0 || version.length === 0 || session > 4) {
    ...
    if (version === "Fast") {
      ...
      num_trials_hand = 60;
      time_limit = 2.1;
      too_late_tol = 0.2;  // time_limit - too_late_tol = 1.9 = beep audio length (0.1s beep lead in + 1.2 s beep length) = accurate response time; 
      timing_tol_early = 0.1;
      timing_tol_late = 0.075;
    }
  ...
  if (version === "Fast") {
    ...
    // randomize prep-time so that prep-time for each symbol spread over a good range
    sample_num = [1, 1, 2, 2, 1, 1, 1, 1, 1, 1, 1, 1, 1];
    for (var i = 0; i<sample_num.length; ++i) {
      let LEN = sample_num[i];
      let tmp = new Array(LEN).fill(i);
      prep_time_ind_tmp = prep_time_ind_tmp.concat(tmp);
    ...
    prep_time_interval = [[prep_time_range[0], 0.1], [0.1, 0.2], [0.2, 0.3], [0.3, 0.4], [0.4, 0.5], [0.5, 0.6], [0.6, 0.7], [0.7, 0.8], [0.8, 0.9], [0.9, 1], [1, 1.1], [1.1, 1.3], [1.3, prep_time_range[1]]];
  }
...
Shape_Background = new visual.ImageStim({
    win : psychoJS.window,
...
function RT_New_BoolLoopBegin(thisScheduler) {
  ...
  RT_New_Bool = new TrialHandler({
    psychoJS: psychoJS,
    nReps: rt_new_yes, method: TrialHandler.Method.SEQUENTIAL,
    extraInfo: expInfo, originPath: undefined,
    trialList: undefined,
...
function Init_StimRoutineBegin(trials) {
  return function () {
    ...
    if (["right", "Right", "RIGHT"].includes(handedness)){
      ...
      if (["left", "Left", "LEFT"].includes(handedness)) {
        ...
        shape_background = StimList[1]["Shape_Background"];
      }
    }
    ...
    for (var i = 0, _pj_a = num_symb; (i < _pj_a); i += 1) {
        ...
        if (["right", "Right", "RIGHT"].includes(handedness)){
          ...
          x_rec.push(StimList[i]["X_pos_rec"]);
          y_rec.push(StimList[i]["Y_pos_rec"]);
          position_rec.push([StimList[i]["X_pos_rec"], StimList[i]["Y_pos_rec"]]);
        } else {
          if (["left", "Left", "LEFT"].includes(handedness)) {
            x.push(StimList[i]["X_pos_rev"]);
            y.push(StimList[i]["Y_pos_rev"]);
            x_rec.push(StimList[i]["X_pos_rec_rev"]);
            y_rec.push(StimList[i]["Y_pos_rec_rev"]);
            position_rec.push([StimList[i]["X_pos_rec_rev"], StimList[i]["Y_pos_rec_rev"]]); 
          }
        }
    }
  ...
  //  symb_remap_ind[remap_pair_2[1]] = symb_map_ind[remap_pair_2[0]];
    ...
    psychoJS.experiment.addData("Set", set);
    // for shape stimuli
    for (var i = 0, _pj_a = num_symb; (i < _pj_a); i += 1) {
      shape_map.push(position_rec[symb_map_ind[i]]);
      shape_remap.push(position_rec[symb_remap_ind[i]]);
    }
    // keep track of which components have finished
    Init_StimComponents = [];
...
var feedback_tooearly_text;
var feedback_toolate_text;
...
Press (space bar) to continue.`
    ;
      ...
      instr_exp_text_extensive = `Welcome back!


Press (space bar) to continue.`
    ;

    instr_exp_text_minimum = `Welcome back!
...
Press (H, U, I, or L) to continue.`
    ;
          ...
          instr_tr_text_hand = `In the following blocks, you will see a hand appear on the screen and hear 4 beeps. You need to press the corresponding finger ON the fourth beep. 
...
In the upcoming block, you will see four squares on the screen, one at a time. Each sqaure spatially corresponds to one of (H, U, I, L). 
    
Your job is to press the spatially-matched finger to respond.
...
Ready? Press (H, U, I, or L) to continue.`
    ...
    if ((training_vol === 'Minimum' && session === 1) || (training_vol === 'Extensive' && session === 3)) {
      instr_rt_new_text = `In the following block, you are going to use the new square-key map you learned. 
...
Whenever you are ready, press (H, U, I, or L) to start.`
      ...
      instr_rt_new_text = `In the following blocks, you are going to practice the new square-key map you learned yesterday. 
...
Whenever you are ready, press (H, U, I, or L) to start.`
    ...
    if ((training_vol === 'Extensive' && session === 1)) {
        instr_rt_text = `Now you are going to practice the square-key map you learned. 
...
There will be ${rt_block} block.
    

Whenever you are ready, press (H, U, I, or L) to start.`
    } else {
        if ((session === 2)) {
            instr_rt_text = `Now, we continue to practice the square-key maps for ${rt_block} blocks.
...
Whenever you are ready, press (H, U, I, or L) to start.`
    ;
            ...
            if (session === 3){
              instr_rt_text = `Now we continue to practice the square-key map you did yesterday for ${rt_block} block.
...
In the following ${tr_block_old} blocks, use the square-key map you learned. Press the corresponding key ON the fourth beep. 
...
In the following ${tr_block_old} blocks, use the square-key map you practices. Press the corresponding key ON the fourth beep. 
...
Now, you will see the same four squares, but this time they may correspond with a different key (H, U, I, L).
...
In the last ${tr_block_new_swap} blocks, use the square-key map you just practiced: 
Press the corresponding key ON the fourth beep. 
...
Press (H, U, I, or L) to accept a 2 second penalty.`
    ...
    feedback_toolate_text = `too late`;
    feedback_tooearly_text = `too early`;
...
var seq_shapey;
var seq_key_shape;
var seq_keynum_shape;
var seq_stimnum_shape;
var seq_position_shape;
...
function Creat_StimSeqRoutineBegin(trials) {
  return function () {
    ...
    if (((block_type !== "CR") && (stim_type === "Shape"))) {
        count = 0;
    ...
    TR_Feedback_Buzz = new sound.Sound({
      win: psychoJS.window,
      value: buzz,
      secs: -1,
      });
    ...
    TR_Feedback_Buzz_Hand = new sound.Sound({
      win: psychoJS.window,
      value: buzz,
      secs: -1,
      });
...
function TR_Enter_Trials_HandRoutineBegin(trials) {
  return function () {
   ...
   // TR_Enter_Trials_HandComponents.push(TR_Rec_Frame_Hand);
    ...
    TR_Enter_Trials_HandComponents.push(TR_Buzz_Hand);
  ...
  return function () {
   ...
   // }
    ...
    if (t >= 0.0 && check_already === 0 && _TR_Press_Hand_allKeys.length > 0) {
      ...
      if ((TR_Press_Hand.keys !== undefined)) {
        if ((TR_Press_Hand.rt < ((time_limit - too_late_tol) - 0.3))) {
            tr_press_tooearly = 1;
            tr_penalty = 1;
            tr_feedback = 1; // for new version where too late and too eary are given a buzzer tone rather than a penalty page.
        } else {
            if ((TR_Press_Hand.rt > ((time_limit - too_late_tol) + 0.2))) {
              tr_press_toolate = 1;
              tr_penalty = 1;
              tr_feedback = 1; // same as above
            } else {
              tr_feedback = 1;
      ...
      if (TR_Press_Hand.corr === 1 && tr_press_early === 0 && tr_press_late === 0 && tr_feedback === 1 && tr_press_tooearly === 0 && tr_press_toolate === 0) {
        play_coin = 1; //sound_vol = 1;
      } else if (tr_press_tooearly === 1 && tr_feedback === 1) {
        play_buzz = 1;
      }
      //feedback_dur = 0.4;
      tr_text_color = [1, 1, 1];
      if ((tr_penalty === 1)) {
          if ((tr_press_toolate === 1)) {
              ...
              tr_text = feedback_toolate_text;
          } else {
              if ((tr_press_tooearly === 1)) {
                  tr_penalty_text = penalty_tooearly_text;
                  tr_text = feedback_tooearly_text;
              }
          }
      } else {
          if ((tr_feedback === 1)) {
            ...
            } else if ((tr_press_early === 1)) {
              tr_text = feedback_early_text;
            } else if (tr_press_tooearly === 1) {
              tr_text = feedback_tooearly_text;
            } else if (tr_press_toolate === 1) {
              tr_text = feedback_toolate_text;
            } else {
    ...
    if (play_coin === 1 && play_buzz === 0) {
      if (t >= 0 && TR_Coin_Hand.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
      ...
      if (t >= frameRemains && TR_Buzz_Hand.status === PsychoJS.Status.NOT_STARTED) {
        TR_Buzz_Hand.tStart = t;  // (not accounting for frame time here)
        TR_Buzz_Hand.frameNStart = frameN;  // exact frame index
        
        //psychoJS.window.callOnFlip(function(){ TR_Coin_Hand.play(); });  // screen flip
        TR_Buzz_Hand.status = PsychoJS.Status.STARTED;
      }

      if (t >= frameRemains && TR_Buzz_Hand.status === PsychoJS.Status.STARTED) {
        //TR_Coin_Hand.stop();  // stop the sound (if longer than duration)
        TR_Buzz_Hand.status = PsychoJS.Status.FINISHED;
      }

    } else if (play_coin === 0 && play_buzz === 1) {
      if (t >= 0 && TR_Buzz_Hand.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        TR_Buzz_Hand.tStart = t;  // (not accounting for frame time here)
        TR_Buzz_Hand.frameNStart = frameN;  // exact frame index
        
        psychoJS.window.callOnFlip(function(){ TR_Buzz_Hand.play(); });  // screen flip
        TR_Buzz_Hand.status = PsychoJS.Status.STARTED;
      }

      if (TR_Buzz_Hand.tStart + TR_Buzz_Hand.getDuration() < time_limit){
        frameRemains = time_limit  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      } else {
        frameRemains = TR_Buzz_Hand.tStart + TR_Buzz_Hand.getDuration()  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      }

      if (t >= frameRemains && TR_Buzz_Hand.status === PsychoJS.Status.STARTED) {
        TR_Buzz_Hand.stop();  // stop the sound (if longer than duration)
        TR_Buzz_Hand.status = PsychoJS.Status.FINISHED;
      }
    ...
    } else if (play_coin === 0 && play_buzz === 0) {
      frameRemains = time_limit  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      if (t >= frameRemains && TR_Buzz_Hand.status === PsychoJS.Status.NOT_STARTED) {
        TR_Buzz_Hand.tStart = t;  // (not accounting for frame time here)
        TR_Buzz_Hand.frameNStart = frameN;  // exact frame index
...
function TR_Enter_Trials_HandRoutineEnd(trials) {
  return function () {
    ...
    if ((tr_penalty === 1)) {
        if ((tr_press_toolate === 1)) {
            tr_penalty_text = penalty_toolate_text;
            tr_feedback_text = feedback_toolate_text;
        } else {
            if ((tr_press_tooearly === 1)) {
                tr_penalty_text = penalty_tooearly_text;
                tr_feedback_text = feedback_tooearly_text;
            }
        }
    } else {
        if ((tr_feedback === 1)) {
    ...
    if (play_coin === 1 && play_buzz === 0){
        if (TR_Coin_Hand.tStart + TR_Coin_Hand.getDuration() <= 0.4){
          feedback_dur_within = time_limit - (TR_Coin_Hand.tStart + TR_Coin_Hand.getDuration()) ;
        } else {
          feedback_dur_within = 0.4;
        } 
    } else if (play_coin === 0 && play_buzz === 1) {
        if (time_limit <= (TR_Buzz_Hand.tStart + TR_Buzz_Hand.getDuration())){
          //feedback_dur_within = (TR_Buzz_Hand.tStart + TR_Buzz_Hand.getDuration()) - time_limit;
          feedback_dur_within = 0.4;
        } else {
          feedback_dur_within = 0.4;
        } 
    } else if (play_coin === 0 && play_buzz === 0 && tr_press_toolate === 0){
      if (tr_penalty === 0){
        if (time_limit - TR_Press_Hand.rt <= 0.4) {
          feedback_dur_within = time_limit - TR_Press_Hand.rt;
        } else if (time_limit - TR_Press_Hand.rt > 0.4) {
          feedback_dur_within = 0.4;
        }  
      }
    } else if (play_coin === 0 && play_buzz === 0 && tr_press_toolate === 1) {
      feedback_dur_within = 0;
    }
    
    feedback_dur = feedback_dur_total - feedback_dur_within;  // feedback_dur_total is 0.1 longer than 0.4, so that there is a 0.1 interval between trials
...
function TR_Feedback_HandRoutineBegin(trials) {
  return function () {
    ...
    TR_Feedback_Buzz_Hand.setVolume(sound_vol);
  ...
  return function () {
    ...
    if (tr_press_toolate === 1) {
      if (t >= 0 && TR_Feedback_Buzz_Hand.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        TR_Feedback_Buzz_Hand.tStart = t;  // (not accounting for frame time here)
        TR_Feedback_Buzz_Hand.frameNStart = frameN;  // exact frame index
        
        psychoJS.window.callOnFlip(function(){ TR_Feedback_Buzz_Hand.play(); });  // screen flip
        TR_Feedback_Buzz_Hand.status = PsychoJS.Status.STARTED;
      }

      if (TR_Feedback_Buzz_Hand.tStart + TR_Feedback_Buzz_Hand.getDuration() < feedback_dur){
        frameRemains = feedback_dur  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      } else {
        frameRemains = TR_Feedback_Buzz_Hand.tStart + TR_Feedback_Buzz_Hand.getDuration()  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      }

      if (t >= frameRemains && TR_Feedback_Buzz_Hand.status === PsychoJS.Status.STARTED) {
        TR_Feedback_Buzz_Hand.stop();  // stop the sound (if longer than duration)
        TR_Feedback_Buzz_Hand.status = PsychoJS.Status.FINISHED;
      }
    } else if (tr_press_toolate === 0) {
      frameRemains = 0.0 + feedback_dur - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      if (t >= 0 && TR_Feedback_Buzz_Hand.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        TR_Feedback_Buzz_Hand.tStart = t;  // (not accounting for frame time here)
...
function Pre_TrialRoutineBegin(trials) {
  return function () {
    ...
    key_item = seq_key_shape[trial_count];
    keynum_item = seq_keynum_shape[trial_count];
    stimnum_item = seq_stimnum_shape[trial_count];
    position_item = seq_position_shape[trial_count];
    if ((block_type === "TR")) {
    ...
    // keep track of which components have finished
    Pre_TrialComponents = [];
    Pre_TrialComponents.push(Shape_Background);
...
function Pre_TrialRoutineEachFrame(trials) {
  return function () {
    ...
    // *Shape_Background* updates
    if (t >= 0.0 && Shape_Background.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Shape_Background.tStart = t;  // (not accounting for frame time here)
      Shape_Background.frameNStart = frameN;  // exact frame index
      
      Shape_Background.setAutoDraw(true);
    }

    frameRemains = 0.0 + pre_trial_dur - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (Shape_Background.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      Shape_Background.setAutoDraw(false);
    }
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
      return quitPsychoJS('The [Escape] key was pressed. Goodbye!', false);
...
function Pre_TrialRoutineEnd(trials) {
  return function () {
    ...
    if ((stim_type === "Hand" || stim_type === "Shape")) {
      psychoJS.experiment.addData("stim_val", stimnum_item);
    } else {
      psychoJS.experiment.addData("stim_val", symb_item);
    }
...
function RT_Enter_TrialRoutineBegin(trials) {
  return function () {
    ...
    RT_Enter_TrialComponents.push(Shape_Background);
    RT_Enter_TrialComponents.push(Stim_Shape);
...
function RT_Enter_TrialRoutineEachFrame(trials) {
  return function () {
    ...
    // *Stim_Shape* updates
    if (t >= 0.0 && Stim_Shape.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Stim_Shape.tStart = t;  // (not accounting for frame time here)
      Stim_Shape.frameNStart = frameN;  // exact frame index
      
      Stim_Shape.setAutoDraw(true);
    }
...
function RT_FeedbackRoutineBegin(trials) {
  return function () {
    ...
    RT_FeedbackComponents.push(Shape_Background);
    RT_FeedbackComponents.push(Stim_Shape);
...
function RT_FeedbackRoutineEachFrame(trials) {
  return function () {
    ...
    frameRemains = 0.0 + feedback_dur - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (Stim_Shape.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      Stim_Shape.setAutoDraw(false);
    }
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
      return quitPsychoJS('The [Escape] key was pressed. Goodbye!', false);
...
function TR_Enter_TrialsRoutineBegin(trials) {
  return function () {
    ...
    TR_Enter_TrialsComponents.push(Shape_Background);
    TR_Enter_TrialsComponents.push(Stim_Shape);
    TR_Enter_TrialsComponents.push(TR_Press);
    TR_Enter_TrialsComponents.push(TR_Coin);
    TR_Enter_TrialsComponents.push(TR_Buzz);
...
function TR_Enter_TrialsRoutineEachFrame(trials) {
  return function () {
    ...
    // *Stim_Shape* updates
    if (t >= stim_start_time && Stim_Shape.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Stim_Shape.tStart = t;  // (not accounting for frame time here)
      Stim_Shape.frameNStart = frameN;  // exact frame index
    ...
    if (t >= 0.0 && check_already === 0 && _TR_Press_allKeys.length > 0) {
      ...
      if (TR_Press.corr === 1 && tr_press_early === 0 && tr_press_late === 0 && tr_feedback === 1 && tr_press_toolate === 0 && tr_press_tooearly === 0) {
    ...
    if (play_coin === 1 && play_buzz === 0) {
      ...
      if (t >= frameRemains && TR_Buzz.status === PsychoJS.Status.NOT_STARTED) {
        TR_Buzz.tStart = t;  // (not accounting for frame time here)
        TR_Buzz.frameNStart = frameN;  // exact frame index
        
        //psychoJS.window.callOnFlip(function(){ TR_Coin.play(); });  // screen flip
        TR_Buzz.status = PsychoJS.Status.STARTED;
      }

      if (t >= frameRemains && TR_Buzz.status === PsychoJS.Status.STARTED) {
        //TR_Coin.stop();  // stop the sound (if longer than duration)
        TR_Buzz.status = PsychoJS.Status.FINISHED;
      }

    } else if (play_coin === 0 && play_buzz === 1) {
      if (t >= 0 && TR_Buzz.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        TR_Buzz.tStart = t;  // (not accounting for frame time here)
        TR_Buzz.frameNStart = frameN;  // exact frame index
        
        psychoJS.window.callOnFlip(function(){ TR_Buzz.play(); });  // screen flip
        TR_Buzz.status = PsychoJS.Status.STARTED;
      }

      if (TR_Buzz.tStart + TR_Buzz.getDuration() < time_limit){
        frameRemains = time_limit  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      } else {
        frameRemains = TR_Buzz.tStart + TR_Buzz.getDuration()  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      }

      if (t >= frameRemains && TR_Buzz.status === PsychoJS.Status.STARTED) {
        TR_Buzz.stop();  // stop the sound (if longer than duration)
        TR_Buzz.status = PsychoJS.Status.FINISHED;
      }
...
function TR_Enter_TrialsRoutineEnd(trials) {
  return function () {
    ...
    if (play_coin === 1 && play_buzz === 0){
      if (TR_Coin.tStart + TR_Coin.getDuration() <= 0.4){
        feedback_dur_within = time_limit - (TR_Coin.tStart + TR_Coin.getDuration()) ;
      } else {
        feedback_dur_within = 0.4;
      } 
    } else if (play_coin === 0 && play_buzz === 1) {
        if (time_limit <= (TR_Buzz.tStart + TR_Buzz.getDuration())){
          //feedback_dur_within = (TR_Buzz_Hand.tStart + TR_Buzz_Hand.getDuration()) - time_limit;
          feedback_dur_within = 0.4;
        } else {
          feedback_dur_within = 0.4;
      ...
      if (tr_penalty === 0){
        if (time_limit - TR_Press.rt <= 0.4) {
          feedback_dur_within = time_limit - TR_Press.rt;
        } else if (time_limit - TR_Press.rt > 0.4) {
          feedback_dur_within = 0.4;
        }  
      }
    } else if (play_coin === 0 && play_buzz === 0 && tr_press_toolate === 1) {
...
function TR_FeedbackRoutineBegin(trials) {
  return function () {
    ...
    TR_Feedback_Buzz.setVolume(sound_vol);
    Shape_Background.setImage(shape_background);
    Stim_Shape.setPos(position_item);
    Stim_Shape.setFillColor(new util.Color(feedback_col));
    Stim_Shape.setLineColor(new util.Color(feedback_col));
    ...
    TR_FeedbackComponents.push(TR_Feedback_Buzz);
    TR_FeedbackComponents.push(Shape_Background);
    TR_FeedbackComponents.push(TR_Feedback_Text);
    TR_FeedbackComponents.push(Stim_Shape);
...
function TR_FeedbackRoutineEachFrame(trials) {
  return function () {
    ...
    if (tr_press_toolate === 1) {
      if (t >= 0 && TR_Feedback_Buzz.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        TR_Feedback_Buzz.tStart = t;  // (not accounting for frame time here)
        TR_Feedback_Buzz.frameNStart = frameN;  // exact frame index
        
        psychoJS.window.callOnFlip(function(){ TR_Feedback_Buzz.play(); });  // screen flip
        TR_Feedback_Buzz.status = PsychoJS.Status.STARTED;
      }

      if (TR_Feedback_Buzz.tStart + TR_Feedback_Buzz.getDuration() < feedback_dur){
        frameRemains = feedback_dur  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      } else {
        frameRemains = TR_Feedback_Buzz.tStart + TR_Feedback_Buzz.getDuration()  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      }

      if (t >= frameRemains && TR_Feedback_Buzz.status === PsychoJS.Status.STARTED) {
        TR_Feedback_Buzz.stop();  // stop the sound (if longer than duration)
        TR_Feedback_Buzz.status = PsychoJS.Status.FINISHED;
      }
    } else if (tr_press_toolate === 0) {
      frameRemains = 0.0 + feedback_dur - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
      if (t >= 0 && TR_Feedback_Buzz.status === PsychoJS.Status.NOT_STARTED) {
    ...
    // start/stop TR_Feedback_Coin
    if (t >= 0 && TR_Feedback_Coin.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
function Instr_RT_NewRoutineBegin(trials) {
  return function () {
    ...
    frameN = -1;
    // update component parameters for each repeat
    Instr_RT_Text.setText(instr_rt_new_text);
    Instr_RT_Press.keys = undefined;
    Instr_RT_Press.rt = undefined;
    _Instr_RT_Press_allKeys = [];
    block_type = "RT";
